...
Can I find anything out about AdminR github repo?
...
## How many repos?
length(unlist(json_df$url))
```


Note: always the possibility to examine the expected url in a webbrowser. This can
...
We can test out using functions in a package that wrap the REST API, but we can also use cURL.
In the Cytoscape example this can be useful if you are wanting to automate a plugin 
...
curl is a versatile command-line tool for making HTTP requests, and it can be used for various types of requests, not just GET. By default, when you use curl without specifying a request method, it performs a GET request. You can also specify POST or PUT verbs. 
...
There are many different tools for doing translations. 
...
Can use `httr2::req_dry_run()` instead of `httr2::req_perform()` to test out the
